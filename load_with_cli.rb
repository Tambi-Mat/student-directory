...
def print_menu
    ...
    puts "4. Load saved list of students.csv"
...
def input_students
    ...
    date_of_birth = STDIN.gets.strip
...
# we want to load data, of students, from a saved file
def load_students(filename = "students.csv")
    # open the saved file students.csv, only in read ("r") mode
    file = File.open(filename, "r")
    # read over all the lines and iterate over them
    file.readlines.each do |line|
        # we split the lines and assign elements to two variables
        # a process called parallel assignment.
        name, cohort = line.chomp.split(",")
            #we then create a new hash and assign it into @students list
...
# we need to check if the file we want to load actually exists
...
def existing_cohorts
    ...
    cohort_to_display = STDIN.gets.strip
